import unittest
from utils.cargo_calculator import CargoCalculator, get_max_ldm

class TestCargoCalculator(unittest.TestCase):

    def test_get_max_ldm(self):
        self.assertEqual(get_max_ldm("bus"), 4.5)
        self.assertEqual(get_max_ldm("solówka"), 7.3)
        self.assertEqual(get_max_ldm("naczepa"), 13.6)
        self.assertEqual(get_max_ldm("nieznany"), 13.6)  # default fallback

    def test_no_cargo_returns_max_ldm(self):
        calc = CargoCalculator("bus")
    ...
    def test_vehicle_suggestion(self):
        ...
        self.assertEqual(result["vehicle"], "bus")
